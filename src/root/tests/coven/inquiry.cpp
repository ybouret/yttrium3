...
Y_UTEST(coven_inquiry)
{
    ...
    XML::Log xml(std::cerr,verbose);
    {
...

        {
            ...
            Coven::Inquiry<Coven::StandardSurvey> rawStdInq(mu,Coven::InquiryExcludesInitial,0x0);
            rawStdInq->print(std::cerr);
            rawStdInq.output(xml,nr);

            (std::cerr << "-- opt standard inquiry" << std::endl).flush();
            Coven::Inquiry<Coven::StandardSurvey> optStdInq(mu,Coven::InquiryExcludesInitial,Coven::Tribes::Optimizing);
...

        {
            ...
            Coven::Inquiry<Coven::PositiveSurvey> rawPosInq(mu,Coven::InquiryIncludesInitial,0x0);
            rawPosInq->print(std::cerr);
            rawPosInq.output(xml,nr);


            (std::cerr << "-- opt positive inquiry" << std::endl).flush();
            Coven::Inquiry<Coven::PositiveSurvey> optPosInq(mu,Coven::InquiryIncludesInitial,Coven::Tribes::Optimizing);
