...
namespace Yttrium
{
    namespace Calculus
    {
        ...
        {
            static uint8_t * From(uint16_t &x) noexcept; //!< \param x input and workspace \return bytes from x
            static uint8_t * From(uint32_t &x) noexcept; //!< \param x input and workspace \return bytes from x
            static uint8_t * From(uint64_t &x) noexcept; //!< \param x input and workspace \return bytes from x
        };

        //! split to 16-bits
        template <> struct SplitWord:: Make<uint16_t>
        {
            static uint16_t * From(uint32_t &x) noexcept; //!< \param x input and workspace \return words from x
            static uint16_t * From(uint64_t &x) noexcept; //!< \param x input and workspace \return words from x
        };

        //! split to 32-bits
        template <> struct SplitWord:: Make<uint32_t>
        {
            static uint32_t * From(uint64_t &x) noexcept;//!< \param x input and workspace \return dwords from x
        };
